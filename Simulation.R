...
for (step_i in c(2:(number_of_steps+1))) {
  ...
  base_GDP_t_minus_1 <- monetary_base_list[step_i-1] 
  ...
  output_gap_Y_t <- (lambda_param * output_gap_Y_t_minus_1 
                     ...
                     + delta_param * (G_t_minus_1_over_Y_star_t_minus_1)) 
  ...
  if(nominal_rate_t > 0) { ## End of page 94
    ...
    #omo_t <- (base_GDP_t - base_GDP_t_minus_1)
    
  }
  else{ ## Case of i_t == 0
    monetary_base_t <- monetary_base_t_minus_1 
    #omo_t <- omo_GDP_t_minus_1
  }
  ...
  omo_list[step_i] <- omo_t_minus_1 
  monetary_base_list[step_i] <- base_GDP_t_minus_1
  dept_list[step_i] <- dept_t_minus_1
  P_t_list[step_i] <- P_t
  print("Iteration :")
  print(step_i)
}
...
plot(omo_list,type="l")
plot(monetary_base_list,type="l")
plot(dept_list,type="l")